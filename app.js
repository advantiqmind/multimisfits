...
function formatDiscord(text) {
  ...
  t = t.replace(/\s*(?:[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE00}-\u{FE0F}\u{200D}\u{20E3}\u{E0020}-\u{E007F}]\s*)+(<\/h[34]>)/gu, "$1");
  t = t.replace(/\n/g, "<br>");
  return t;
}
...
function renderEvents(events, { cached } = {}) {
  ...
  if (evWinnerWrap) {
    ...
    evWinnerWrap.innerHTML = '<div class="ev-winner-bar"><span class="ev-winner-bar-icon">&#127942;</span><div class="ev-winner-bar-main"><span class="ev-winner-bar-label">Prev. Winner</span><span class="ev-winner-bar-name">' + winnerName + '</span>' + evEventHtml + '</div></div>';
...
function renderGiveaways(rounds, { cached } = {}) {
  ...
  if (gaWinnerWrap) {
    ...
    gaWinnerWrap.innerHTML = '<div class="ev-winner-bar"><span class="ev-winner-bar-icon">&#127942;</span><div class="ev-winner-bar-main"><span class="ev-winner-bar-label">Prev. Winner</span><span class="ev-winner-bar-name">' + gaWinnerName + '</span>' + gaEventHtml + '</div></div>';
